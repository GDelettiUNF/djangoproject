...
def compound_data(request):
    compounddata = Compound_Data.objects.filter(published_date__lte=timezone.now()).order_by('published_date')
    return render(request, 'compounds/compound_data.html', {'compound_data': compounddata})
...
def computed_mw_new(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            post.pubchem_molecular_weight = str(APImw(post.inchikey))
